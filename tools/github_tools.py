...
def create_pull_request(
    ...
    The agent should call git tools to commit/push before this function.
    """
    if not branch_name:
    ...
    try:
        ...
        raise GitHubToolError(
            f"Unable to create Pull Request: GitHub returned {exc.status}. "
            f"Check that the branch exists and the token has repository permissions."
        ) from exc
